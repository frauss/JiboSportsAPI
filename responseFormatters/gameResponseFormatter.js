(function () {
    ...
    function GameResponseFormatter(gameInfo) {
        ...
        this.formulateResponse = function(queryParams) {
            var self = this;
            var gameResponses = [];
            if (queryParams.rule === "whendo") {
                var gameInfo = jmespath.search(this.gameData[0], "game.{ status: status, currentInning: outcome.current_inning, currentInningHalf: outcome.current_inning_half, gameTime: scheduled, venue: venue.name, homeScore: home.runs, awayScore: away.runs, lastInning: final.inning, homeId: home.id, homeName: home.name, awayId: away.id, awayName: away.name, rescheduleInfo: rescheduled}");
                var gameTime = moment(gameInfo.gameTime);
                var responseText = sprintf("The next game between the %s and the %s will be played at %s on %s starting at %s",
                    gameInfo.homeName, gameInfo.awayName, gameInfo.venue,
                    gameTime.format("MMMM D, YYYY"),
